...
export function StatCard({ title, value, icon: Icon, color = 'blue', trend, className }) {
  ...
  return (
    <Card className={clsx('border flex flex-col justify-between h-full', c.border, className)} padding={false}>
      <div className="p-4 sm:p-5 flex flex-col justify-between h-full min-h-[110px] sm:min-h-[125px]">
        {/* Top section: Title and Icon */}
        <div className="flex items-start justify-between gap-3">
          <p className="text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider min-h-[32px] sm:min-h-[36px] flex items-center leading-tight">
            {title}
          </p>
          <div className={clsx('w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm', c.bg)}>
